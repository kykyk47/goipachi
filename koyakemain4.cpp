...
void display(void)
{
	...
	case 5:
	{
		...
		else
		{
			switch (walk_timer)
			{
			case 1:  if (player.direction == 1) { sample->ChangeImage(2); }
					 else { sample->ChangeImage(7); } break;//glBindTexture(GL_TEXTURE_2D, player3.tex); break;
			case 2:  if (player.direction == 1) { sample->ChangeImage(1); }
					 else { sample->ChangeImage(6); } break;//glBindTexture(GL_TEXTURE_2D, player2.tex); break;
			case 3:  if (player.direction == 1) { sample->ChangeImage(0); }
					 else { sample->ChangeImage(5); } break;//glBindTexture(GL_TEXTURE_2D, player1.tex); break;
			case 0:  if (player.direction == 1) { sample->ChangeImage(1); }
					 else { sample->ChangeImage(6); } break;// glBindTexture(GL_TEXTURE_2D, player2.tex); break;
			}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'm':
	{
		if (full_screen == false) { full_screen = true; }
...
void Init() {
	...
	for (i = 1; i <= 79; i++)
	{
		...
		printf("%d to %d : %d\n", choose_hiragana_weight_add[i - 1], choose_hiragana_weight_add[i] - 1, i);
