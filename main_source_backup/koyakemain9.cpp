...
void game_shutdown(void) //Escキーを押下されたとき諸々を終了させる
{
	...
	Mix_FreeChunk(SE_select); std::cout  << "<info 057: SDLを閉じました>" << std::endl;
	Mix_CloseAudio(); std::cout  << "<info 058: SFL_Mixerを閉じました>" << std::endl;
...
void display(void)
{
	...
	case 5:
	{
		...
		if (mode == 1) //ステージモードのとき，今まで作った単語を左上に表示
		{
			for (i = 0; i <= 100; i++) 
			{
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 5: //ゲーム画面
	{
		...
		case 'j': {  if (slot_select > 0 && lamp_timer_02 == 0) { Mix_PlayChannel(-1, SE_select, 0);  slot_select--; }} break; //スロット移動
		case 'l': {  if (slot_select < 3 && lamp_timer_02 == 0) { Mix_PlayChannel(-1, SE_select, 0);  slot_select++; }} break;
		...
		case 'k':
		{
			if (lamp_timer_02 == 0 && slot[0] != 0 && slot[1] != 0 && slot[2] != 0 && slot[3] != 0)
			{
				check_goi(slot);
				lamp_timer_02 = 100;
				lamp_timer_01 = 50;
	...
	case 7: //モード選択画面
	{
		...
		case 'l': {	if (mode == 0) { Mix_PlayChannel(-1, SE_enter, 0);  stage_select = 0;  scene = 8;  } if (mode == 1) { stage_select = 1;  scene = 9; }} break; //スコアアタックモードはステージ０として扱う
...
void Init() {
	...
	i = 0;
	while (fscanf_s(fp_stage_nolma_info, "%d,%d,%d,%d,%d,%d,%d,%d", &stage_info[i], &stage_nolma[i], &stage_time_limit[i], &stage_time_limit_gold[i], &stage_slot_constraint[i][0], &stage_slot_constraint[i][1], &stage_slot_constraint[i][2], &stage_slot_constraint[i][3]) != EOF)
	{
		i++;
	}
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("goipachi ver.1.2.0");
