...
int **dic_3moji; //辞書の情報を格納
int **dic_4moji; //辞書の情報を格納
int **dic_5moji; //辞書の情報を格納
//int dic_sample[][5] = { {} }; //辞書の情報を格納
...
void check_goi(int* moji)
{
	...
	int **dic3 = &dic_3moji[0];
	int **dic4 = &dic_4moji[0];
	int **dic5 = &dic_5moji[0];
	...
	case 0: //スコアアタックモードのとき
	{
		...
		case 3:
		{
			...
			{
				if (moji[0] == (*(*(dic3 + i) + 0)) && moji[1] == (*(*(dic3 + i) + 1)) && moji[2] == (*(*(dic3 + i) + 2)))
				{
					word_hit = true;
					play_SE(SE_maru);
		...
		case 4:
		{
			...
			{
				if (moji[0] == (*(*(dic4 + i) + 0)) && moji[1] == (*(*(dic4 + i) + 1)) && moji[2] == (*(*(dic4 + i) + 2)) && moji[3] == (*(*(dic4 + i) + 3)))
				{
					word_hit = true;
					play_SE(SE_maru);
		...
		case 5:
		{
			...
			{
				if (moji[0] == (*(*(dic5 + i) + 0)) && moji[1] == (*(*(dic5 + i) + 1)) && moji[2] == (*(*(dic5+ i) + 2)) && moji[3] == (*(*(dic5 + i) + 3)) && moji[4] == (*(*(dic5 + i) + 4)))
				{
					word_hit = true;
					play_SE(SE_maru);
...
void game_shutdown(void)//Escキーやタイトル→やめるを押下されたとき諸々を終了させる
{
	...
	for (i = 0; i <= dic_3_all; i++) { delete[] dic_3moji[i]; } delete[] dic_3moji;
	for (i = 0; i <= dic_4_all; i++) { delete[] dic_4moji[i]; } delete[] dic_4moji;
	for (i = 0; i <= dic_5_all; i++) { delete[] dic_5moji[i]; } delete[] dic_5moji;
}
...
void Init() {
	...
	if ((fopen_s(&fp_dic_3, "./dat/3moji_dic.dat", "r")) != 0) //3文字辞書ファイルを読み込む
	{
	...
	while (fscanf_s(fp_dic_3, "%d,%d,%d,", &dic_temp, &dic_temp, &dic_temp) != EOF) //まず辞書の配列に必要な要素数を取得
	{
		i++;
	}

	dic_3_all = i;
	dic_3moji = new int*[dic_3_all]; //辞書の配列を動的に確保
	i = 0;
	dic_3moji[0] = new int[3];

	fclose(fp_dic_3);
	if ((fopen_s(&fp_dic_3, "./dat/3moji_dic.dat", "r")) != 0)  //ファイルを開きなおす
	{
		std::cout << "<info 012: 辞書ファイルを開けませんでした" << std::endl;
		exit(12);
	}
	...
	while (fscanf_s(fp_dic_3, "%d,%d,%d,", &dic_3moji[i][0], &dic_3moji[i][1], &dic_3moji[i][2]) != EOF) 
	{
		//printf("%d,%d,%d\n",dic_3moji[i][0], dic_3moji[i][1], dic_3moji[i][2]);
		i++;
		dic_3moji[i] = new int[3]; //辞書の配列を動的に確保
	}
	...
	if ((fopen_s(&fp_dic_4, "./dat/4moji_dic.dat", "r")) != 0) //4文字辞書ファイルを読み込む
	{
		std::cout << "<info 012: 辞書ファイルを開けませんでした" << std::endl;
		exit(12);
	}
	...
	while (fscanf_s(fp_dic_4, "%d,%d,%d,%d,", &dic_temp, &dic_temp, &dic_temp, &dic_temp) != EOF) //まず辞書の配列に必要な要素数を取得
	{
		i++;
	}

	dic_4_all = i;
	dic_4moji = new int*[dic_4_all]; //辞書の配列を動的に確保
	i = 0;
	dic_4moji[0] = new int[4];

	fclose(fp_dic_4);
	if ((fopen_s(&fp_dic_4, "./dat/4moji_dic.dat", "r")) != 0)  //ファイルを開きなおす
	{
		std::cout << "<info 012: 辞書ファイルを開けませんでした" << std::endl;
		exit(12);
	}
	...
	while (fscanf_s(fp_dic_4, "%d,%d,%d,%d,", &dic_4moji[i][0], &dic_4moji[i][1], &dic_4moji[i][2], &dic_4moji[i][3]) != EOF)
	{
		//printf("%d,%d,%d,%d\n", dic_4moji[i][0], dic_4moji[i][1], dic_4moji[i][2], dic_4moji[i][3]);
		i++;
		dic_4moji[i] = new int[4]; //辞書の配列を動的に確保
	}
	...
	if ((fopen_s(&fp_dic_5, "./dat/5moji_dic.dat", "r")) != 0) //5文字辞書ファイルを読み込む
	{
		std::cout << "<info 012: 辞書ファイルを開けませんでした" << std::endl;
		exit(12);
	}
	...
	while (fscanf_s(fp_dic_5, "%d,%d,%d,%d,%d,", &dic_temp, &dic_temp, &dic_temp, &dic_temp, &dic_temp) != EOF) //まず辞書の配列に必要な要素数を取得
	{
		i++;
	}

	dic_5_all = i;
	dic_5moji = new int*[dic_5_all]; //辞書の配列を動的に確保
	i = 0;
	dic_5moji[0] = new int[5];

	fclose(fp_dic_5);
	if ((fopen_s(&fp_dic_5, "./dat/5moji_dic.dat", "r")) != 0)  //ファイルを開きなおす
	{
		std::cout << "<info 012: 辞書ファイルを開けませんでした" << std::endl;
		exit(12);
	}
	...
	while (fscanf_s(fp_dic_5, "%d,%d,%d,%d,%d,", &dic_5moji[i][0], &dic_5moji[i][1], &dic_5moji[i][2], &dic_5moji[i][3], &dic_5moji[i][4]) != EOF)
	{
		//printf("%d,%d,%d,%d\n", dic_5moji[i][0], dic_5moji[i][1], dic_5moji[i][2], dic_5moji[i][3], dic_5moji[i][4]);
		i++;
		dic_5moji[i] = new int[5]; //辞書の配列を動的に確保
	}
	...
	for (k = 0; k < dic_3_all; k++)
	{
		for (j = 0; j <= 2; j++)
		{
			(*(hw3 + *(*(dic3 + k)+j)*3+j))++; //ひらがなの登場回数を計算して格納する
			//printf("%d\n", *(*(dic3 + k)+j));
		}
	}
	...
	for (k = 0; k < dic_4_all; k++)
	{
		for (j = 0; j <= 3; j++)
		{
			(*(hw4 + *(*(dic4 + k) + j)*4+j))++; //ひらがなの登場回数を計算して格納する
		}
	}
	...
	for (k = 0; k < dic_5_all; k++)
	{
		for (j = 0; j <= 4; j++)
		{
			(*(hw5 + *(*(dic5 + k) + j)*5+j))++; //ひらがなの登場回数を計算して格納する
		}
	}
	...
	for (k = 0; k < 80; k++) //何文字目の何のひらがなであれば何点，という点数を付与する
	{
		for (j = 0; j <= 2; j++)
		{
			...
			else if (*(hw3 + 3 * k + j) >= 1) { *(hs3 + 3 * k + j) = 45; }
			else { *(hs3 + 3 * k + j) = -1; }
		}
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("goipachi ver.1.5.2");
