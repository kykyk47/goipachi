...
//ポインタの名前は変数(または配列)名の略称っぽいもの（例：stage_structure　→　stst）で統一した

//★メモ1：スコアアタックモードにおいて，地形パターンを増やす手順
//1.choose_pattern()の各文字のdo-whileの条件を変える
//2.block_standbyでcase 0～,100～,200～なり新しく追加する

//★メモ2：スコアアタックモードにおいて，ヒントブロックからでる穴あきワードのパターンを増やす手順
//1.odai_hiragana_* の配列に新しいパターンを追加記述する
//2.odai_hiragana_*_all を増やす

//★メモ3：ステージクリアモードのステージを新しく作る
//1.同梱の「goipachi_studio」を用いて制作する（操作方法はすべてツール内のUIに補助記述済）
...
#define STAGE_AVAILABLE 100 //★提出段階で見せられるステージの数
...
int stage_medal[3] = {}; //全ステージ中の獲得したメダルの総数を記録（メニュー画面でステージモードのクリア進捗を表示するのに使う）
...
int stage_info[STAGE_LIMIT + 1] = {}; //0:未定義 3-5:スロット固定あり言葉作成ミッション（固定情報がなければ通常単語作成ミッション），13-15：しりとりミッション， 23-25：ABには同じ言葉が入りますミッション
...
std::mt19937 mt(rnd()); //乱数のタネを決定するやつ
...
class GameObject  //大元となるクラス．★M担当部分
{
public:
	double center_x = 0;
	double center_y = 0;
...
GameObject UI_block_coming_soon = GameObject(0, 0, 96, 96, L"./pic/coming_soon_block.png"); //ステージ選択画面でのステージ番号の背景のブロック(プレイ可能なステージの上限の→にくっついているカミングスーン)
...
GameObject(0, 0, 96, 96, L"./pic/block_undifined.png"), //未使用
GameObject(0, 0, 96, 96, L"./pic/block_undifined.png"), //未使用
GameObject(0, 0, 96, 96, L"./pic/block_undifined.png") //未使用
};
...
void check_goi(int* moji) //スロットに取得したひらがながそろったとき，その単語が辞書に登録されているかをチェックする
{
	...
	case 0: //スコアアタックモードのとき
	{
		...
		case 3:
		{
			for (i = 0; i < dic_3_all; i++) //辞書を最初から探索しひらがなが全部一致しているかを確認
			{
				if (moji[0] == *(dic3 + i * 3) && moji[1] == *(dic3 + i * 3 + 1) && moji[2] == *(dic3 + i * 3 + 2)) //合っていればひらがなに応じた得点を加算し，作った単語+1
				{
					word_hit = true;
					play_SE(SE_maru);
			...
			if (word_hit == false) //辞書のどれも一致してなければペナルティ
			{
				play_SE(SE_batsu);
				time -= 30 * 60;
			}
		...
		case 4:
		{
			...
			{
				if (moji[0] == *(dic4 + i * 4) && moji[1] == *(dic4 + i * 4 + 1) && moji[2] == *(dic4 + i * 4 + 2) && moji[3] == *(dic4 + i * 4 + 3)) //合っていればひらがなに応じた得点を加算し，作った単語+1
				{
					word_hit = true;
					play_SE(SE_maru);
		...
		case 5:
		{
			...
			{
				if (moji[0] == *(dic5 + i * 5) && moji[1] == *(dic5 + i * 5 + 1) && moji[2] == *(dic5 + i * 5 + 2) && moji[3] == *(dic5 + i * 5 + 3) && moji[4] == *(dic5 + i * 5 + 4)) //合っていればひらがなに応じた得点を加算し，作った単語+1
				{
					word_hit = true;
					play_SE(SE_maru);
	...
	case 1: //ステージクリアモードのとき
	{
		...
		case 3:
		{
			...
			{
				if (moji[0] == *(dic3 + i * 3) && moji[1] == *(dic3 + i * 3 + 1) && moji[2] == *(dic3 + i * 3 + 2))//辞書の単語の平仮名と一致している場合
				{
					for (k = 0; k <= score; k++)
					{
						if (*(mdtn + k * 5 + 0) == moji[0] && *(mdtn + k * 5 + 1) == moji[1] && *(mdtn + k * 5 + 2) == moji[2]) //今まで作成した単語とかぶっていれば，正解でなくする
						{
							word_hit = false;
							break;
			...
			if (word_hit == false) //既に単語が作成済み or 辞書のどれも一致してなければペナルティ
			{
				play_SE(SE_batsu);
				time -= 30 * 60;
				score_miss++;
		...
		case 4:
		{
			for (i = 0; i < dic_4_all; i++)
			{
				if (moji[0] == *(dic4 + i * 4) && moji[1] == *(dic4 + i * 4 + 1) && moji[2] == *(dic4 + i * 4 + 2) && moji[3] == *(dic4 + i * 4 + 3))//辞書の単語の平仮名と一致している場合
				{
					for (k = 0; k <= score; k++)
					{
						if (*(mdtn + k * 5 + 0) == moji[0] && *(mdtn + k * 5 + 1) == moji[1] && *(mdtn + k * 5 + 2) == moji[2] && *(mdtn + k * 5 + 3) == moji[3])//今まで作成した単語とかぶっていれば，正解でなくする
						{
							word_hit = false;
							break;
		...
		case 5:
		{
			for (i = 0; i < dic_5_all; i++)
			{
				if (moji[0] == *(dic5 + i * 5) && moji[1] == *(dic5 + i * 5 + 1) && moji[2] == *(dic5 + i * 5 + 2) && moji[3] == *(dic5 + i * 5 + 3) && moji[4] == *(dic5 + i * 5 + 4))//辞書の単語の平仮名と一致している場合
				{
					for (k = 0; k <= score; k++)
					{
						if (*(mdtn + k * 5 + 0) == moji[0] && *(mdtn + k * 5 + 1) == moji[1] && *(mdtn + k * 5 + 2) == moji[2] && *(mdtn + k * 5 + 3) == moji[3] && *(mdtn + k * 5 + 4) == moji[4])//今まで作成した単語とかぶっていれば，正解でなくする
						{
							word_hit = false;
							break;
...
void LoadImagePNG(const wchar_t* filename, GLuint &texture) //png画像を読み込む
{
	glEnable(GL_TEXTURE_2D);
	Bitmap bmp(filename);
	BitmapData data;
...
void SetNumImage(double x, double y, int size_x, int size_y, int num, int font, int d) { //numを画像の羅列として表示する．font:1はプレイヤーエフェクトとしての数字(数字の左に「＋」がつく) //dは数字同士の間隔
...
void game_reset(void) //ステージ構造などゲームを開始する直前にゲームを準備する（スコアアタックモード
{
	...
	*(stst + 18) = -2; //最初から18番目のパターンに最後の壁を配置．ステージを無限にどこまでも行けてしまうとヌルゲーになってしまうので
...
void display(void)
{
	...
	case 9: //ステージ選択画面
	{
		...
		for (i = -3; i <= 3; i++)
		{
			if(stage_select+i == STAGE_AVAILABLE +1) //プレイ可能なステージの上限の一個右にCOMING SOON...とかかれたブロックを置いておく
			{
				UI_block_coming_soon.SetImage(-i * 128, -348);
			}
...
void idle(void)
{
	...
	if (player_jump == true && flag_move_y == true && move_lock == false) //ジャンプ
	{
		...
		if (flag_collision_U == true) //天井との衝突を感知したフレームでの処理
		{
			...
			player->Move(0, (-((int)(player->center_y))) % 64); //衝突したらそのブロックの下のブロックまで戻す
	...
	if (onMoveKeyPress_L == true && flag_move_x == true && move_lock == false) { //左に移動
		...
		if (flag_collision_L == true) //横方向にはカメラも追従し，カメラとUIの横方向の位置はプレイヤーの横移動と同じように動く
		{
			camera_x -= speed;
			player->Move(-speed, 0);
		}
	...
	if (onMoveKeyPress_R == true && flag_move_x == true && move_lock == false) { //右に移動
		...
		{
			camera_x += speed; //横方向にはカメラも追従し，カメラとUIの横方向の位置はプレイヤーの横移動と同じように動く
			player->Move(speed, 0);
		}
		...
		for (i = 0; i < object_on_stage; i++)
		{
			if (flag_bullet_exist == true && *(obbl + i * 3) != 0 && abs(bullet->center_x - double(*(obbl + i * 3 + 1))) < 32 && abs(bullet->center_y - double(*(obbl + i * 3 + 2))) < 32) //ブロックとの距離がx<32 y<64で32あるとき（ブロックＩＤ＝０すなわち空気の時はスルー）
			{
					...
					*(sl + slot_select) = *(obbl + i * 3); //弾丸が衝突したブロックをスロットに格納
					...
					if (mode == 1 && *(si + stage_select) >= 23 && *(si + stage_select) <= 25) //A〇A〇のとき
					{
						if (*(ssc + stage_select * 5 + slot_select) != 0) //選択しているスロットがＡとかになってるとき
						{
							...
							for (k = 0; k <= 4; k++) //ステージのスロット固定情報より今選択している語彙スロットの位置の固定情報がほかの場所のスロットの固定情報があればそこにも同じひらがなを入れる
							{
				...
				else if (*(obbl + i * 3) == 77) //お題箱にヒットしたとき（ごいスロットに何かあるときはＯＦＦ状態になる）
				{
					...
					{
						odai = choose_odai(mode_mojisu); //おだいを選ぶ
						*(sl + 0) = *(odhr3 + odai * 5 + 0);
						*(sl + 1) = *(odhr3 + odai * 5 + 1);
						*(sl + 2) = *(odhr3 + odai * 5 + 2);
						play_SE(SE_hit);
	...
	if (scene == 5 && mode == 1 && stage_norma[stage_select] == score) //ステージクリアモードでノルマを達成した
	{
		...
		before_stageclear_miss = *(sc + stage_select * 5 + 4); //リザルト画面でnew recordを表示するために更新前の情報が必要なのでここでとっておく
		before_stageclear_time = *(sc + stage_select * 5 + 3);

		if (*(sc + stage_select * 5 + 4) > score_miss) //ミス回数のハイスコア更新
		{
			*(sc + stage_select * 5 + 4) = score_miss;
		}
		...
		if (*(sc + stage_select * 5 + 3) > (int)((*(stl + stage_select) - (double)(time) / 60) * 100) - 3000 * score_miss || before_stageclear_time == 0) //タイム更新 ファイルの0は未プレイを表している（0秒クリアはありえないので）
		{
			*(sc + stage_select * 5 + 3) = (int)((*(stl + stage_select) - (double)(time) / 60) * 100) - 3000 * score_miss; //◆

		}

		*(sc + stage_select * 5 + 0) = 1; //ノルマクリアで無条件でメダル一枚目獲得

		if (score_miss == 0)
		{
			*(sc + stage_select * 5 + 1) = 1; //ミス０回でメダル二枚目獲得
		}

		if (*(stl + stage_select) - time / 60 - score_miss * 30 <= *(stlg + stage_select))
		{
			*(sc + stage_select * 5 + 2) = 1; //ゴールドタイマーでメダル3枚目獲得
		...
		{
			fprintf(fp_stageclear, "%d,%d,%d,%d,%d\n", *(sc + i * 5 + 0), *(sc + i * 5 + 1), *(sc + i * 5 + 2), *(sc + i * 5 + 3), *(sc + i * 5 + 4)); //ステージクリア進捗状況ファイルを更新
...
void Init() {
	...
	UI_block_coming_soon.LoadImagePNG2(UI_block_coming_soon.file, UI_block_coming_soon.tex);
