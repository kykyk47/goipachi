...
void new_stage(); //エディタを白紙に戻す
void import_stage(int stage_num); //ステージファイルからステージ情報．ブロック配置情報をエディタにインポートする
void export_stage(int stage_num); //ステージファイルへステージ情報，ブロック配置情報をエクスポートする
void shutdown(void); //エディタの終了
int def_hiragana(int x, int y); //カーソルの位置よりエディタ上画面で配置するひらがなのＩＤを決定する
...
class GameObject //★ゲーム本体の該当箇所と同じくM担当
{
public:
	double center_x = 0;
	double center_y = 0;
...
GameObject UI_stage_info_0 = GameObject(0, 0, 192, 48, L"./pic/stage_info_0.png"); //ステージの種類（スロット固定とかしりとりとか）
GameObject UI_stage_info_1 = GameObject(0, 0, 192, 48, L"./pic/stage_info_1.png"); //ステージの種類（スロット固定とかしりとりとか）
GameObject UI_stage_info_2 = GameObject(0, 0, 192, 48, L"./pic/stage_info_2.png"); //ステージの種類（スロット固定とかしりとりとか）
...
void display(void)
{
	...
	{
		UI_stage_info_1.SetImage(-315 + camera_x, 148); //しりとり
	}break;

	case 23: case 24: case 25:
	{
		UI_stage_info_2.SetImage(-315 + camera_x, 148); //スロット制限ＡＢＡＢ
	}break;
	}
	...
	case 23: case 24: case 25:
	{
		slot_same_block_mini[stage_slot_constraint_edit[0]].SetImage(-930 + camera_x + 34 * 20, 266); //固定スロット情報ABAB
		slot_same_block_mini[stage_slot_constraint_edit[1]].SetImage(-930 + camera_x + 34 * 19, 266);
		slot_same_block_mini[stage_slot_constraint_edit[2]].SetImage(-930 + camera_x + 34 * 18, 266);
		slot_same_block_mini[stage_slot_constraint_edit[3]].SetImage(-930 + camera_x + 34 * 17, 266);
		slot_same_block_mini[stage_slot_constraint_edit[4]].SetImage(-930 + camera_x + 34 * 16, 266);
	}break;
	}
			...
			case 's':  //下半分で配置するブロックを選択 or B.xが16以上のときはステージ情報を選択
			{
				if (cursorB.y <= 3) {
					if (cursorB.x == 16 && cursorB.y == 3) {
						cursorB.x = 20;  cursorB.y = 4; select_hiragana = def_hiragana(cursorB.x, cursorB.y);
					}
...
void Init() {
	...
	UI_stage_info_0.LoadImagePNG2(UI_stage_info_0.file, UI_stage_info_0.tex);
	UI_stage_info_1.LoadImagePNG2(UI_stage_info_1.file, UI_stage_info_1.tex);
	UI_stage_info_2.LoadImagePNG2(UI_stage_info_2.file, UI_stage_info_2.tex);
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("goipachi editor ver.1.0.15");
